...
CURRENT_CONFIG="${HOME}/.codex/config.toml"

tmp_dir="$(mktemp -d)"
trap 'rm -rf "$tmp_dir"' EXIT

expected_config="$tmp_dir/expected.toml"
normalized_current="$tmp_dir/current.toml"

python3 - "$COMMON_CONFIG" "$LOCAL_CONFIG" "$expected_config" <<'PY'
import pathlib
import sys
import tomllib
...
echo "== effective diff: merged common/local config vs ~/.codex/config.toml =="
if [ -f "$CURRENT_CONFIG" ]; then
  python3 - "$CURRENT_CONFIG" "$normalized_current" <<'PY'
import pathlib
import sys
import tomllib
...
PY
  ...
  if diff -u "$normalized_current" "$expected_config"; then
    echo "No differences."
  fi
else
